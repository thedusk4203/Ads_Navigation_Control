...
(function() {
    ...
    const secondLevelTLDs = ['co', 'com', 'net', 'org', 'edu', 'gov', 'ac', 'or', 'ne'];
    ...
    function isBlacklisted(domain) {
        ...
        return blacklist.some(blocked => {
            ...
            return domain.toLowerCase().includes(blockedLower) || 
    ...
    console.log('[AdsControl] Script đang hoạt động trên:', realWindow.location.hostname);
    ...
    function showToast(message, blockedUrl = null, type = 'block') {
        ...
        `;
        allowBtn.onmouseenter = () => allowBtn.style.background = 'rgba(255,255,255,0.4) !important';
        allowBtn.onmouseleave = () => allowBtn.style.background = 'rgba(255,255,255,0.25) !important';
        allowBtn.onclick = (e) => {
            e.stopPropagation();
            if (blockedUrl) {
    ...
    function isAllowedIframe(url) {
        try {
            if (!url || url === '' || url === 'about:blank' || url === 'about:srcdoc') {
                return true;
            }
            const parsedUrl = new URL(url, realWindow.location.href);
            return parsedUrl.origin === currentOrigin;
    ...
    realWindow.open = function(url, name, features) {
        ...
        try {
            ...
            showToast('Chặn popup: ' + new URL(url, realWindow.location.href).hostname, fullUrl);
        } catch(e) {
    ...
    document.addEventListener('click', function(e) {
        ...
        if (target && target.href) {
            if (!isAllowedNavigation(target.href)) {
                ...
                try {
                    showToast('Chặn link: ' + new URL(target.href).hostname, target.href);
                } catch(e) {
    ...
    if (realWindow.history) {
        const originalPushState = realWindow.history.pushState.bind(realWindow.history);
        ...
        const originalReplaceState = realWindow.history.replaceState.bind(realWindow.history);
    ...
    try {
        ...
        console.warn('[AdsControl] Không thể override location methods:', e.message);
    }
    ...
    realWindow.addEventListener('beforeunload', function(e) {
    }, true);
    ...
    realWindow.addEventListener('load', function() {
